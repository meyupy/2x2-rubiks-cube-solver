...
def produce_random_scramble():
    ...
    scramble_length = random.randint(9, 10)
    while len(random_scramble) < scramble_length:
        random_notation = random.choice([the_notation for the_notation in NOTATIONS
                                         if the_notation[0] != random_scramble[-1][0]])
        random_scramble.append(random_notation)
...
def is_cube_solved(perm):
    for s in range(0, 24, 4):
        if not (perm[s] == perm[s+1] == perm[s+2] == perm[s+3]):
            return False
    return True
...
def scan(paths, perms):
    ...
    for n in range(len(paths)):
        for notation in NOTATIONS:
            if paths[n][-1][0] != notation[0]:
                new_path = [notation for notation in paths[n]]
...
def solve_the_cube(perm):
    if is_cube_solved(perm):
        return []
    paths = [[notation] for notation in NOTATIONS]
    perms = [produce_permutation(path[-1], perm) for path in paths]
    for n in range(len(paths)):
...
DEFAULT_PERMUTATION = ['g', 'g', 'g', 'g', 'w', 'w', 'w', 'w', 'r', 'r', 'r', 'r',
                       'b', 'b', 'b', 'b', 'y', 'y', 'y', 'y', 'o', 'o', 'o', 'o']

current_permutation = [c for c in DEFAULT_PERMUTATION]
...
while True:
    ...
    for turner_button in turner_buttons:
        if turner_button.is_clicked():
            current_permutation = produce_permutation_for_turner_buttons(turner_button.text, current_permutation)
            if current_permutation == last_scrambled_permutation:
                scramble_surf = gui_font_large.render('  '.join(scramble), True, TEXT_COLOR_1_1)
            else:
                scramble_surf = gui_font_large.render('  '.join(scramble), True, TEXT_COLOR_1_2)
            if current_permutation in last_solution_permutations:
                solution_surf = gui_font_large.render('  '.join(solution), True, TEXT_COLOR_2_1)
            else:
                solution_surf = gui_font_large.render('  '.join(solution), True, TEXT_COLOR_2_2)
    ...
    if button_scramble.is_clicked():
        ...
        last_scrambled_permutation = [c for c in current_permutation]
        scramble_surf = gui_font_large.render('  '.join(scramble), True, TEXT_COLOR_1_1)
        scramble_rect = scramble_surf.get_rect(center=(3 * W // 8, 79 * W // 128))
        solution = []
        last_solution_permutations = []
    ...
    if button_solve.is_clicked():
        if not is_cube_solved(current_permutation) and current_permutation != last_permutation_solved:
            ...
            solution = solve_the_cube(current_permutation)
            last_solution_permutations = produce_permutations_from_path(current_permutation, solution)
            last_permutation_solved = [c for c in current_permutation]
            solution_surf = gui_font_large.render('  '.join(solution), True, TEXT_COLOR_2_1)
            solution_rect = solution_surf.get_rect(center=(3 * W // 8, 89 * W // 128))
            if current_permutation != last_scrambled_permutation:
                scramble = []
